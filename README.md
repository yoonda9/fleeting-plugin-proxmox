...
| `http_timeout`               | int (seconds)             | 60                                      | Per-request deadline for calls to the Proxmox VE API.                                                                                                                                                                                                                                                                |
| `http_max_idle_conns_per_host` | int                      | (derived) `max(8, clone_concurrency + 4)` | Maximum idle HTTP connections to keep open per Proxmox VE host.                                                                                                                                                                                                                                                      |
| `proxmox_api_retry_attempts` | int                        | 3                                        | How many times to retry a read-only Proxmox API call that failed transiently.                                                                                                                                                                                                                                        |
| `clone_concurrency`          | int                        | 4                                        | Maximum number of clone tasks (from the clone request through task completion) in flight at once. Bounds the disk bandwidth a large scale-up takes on one node.                                                                                                                                                     |
| `clone_bandwidth_limit`      | int (KiB/s)                | (None - uses the Proxmox VE datacenter/storage default) | Maximum bandwidth a single clone task may use. Zero or unset preserves Proxmox VE's configured default.                                                                                                                                                                                                              |
| `vmid_range_start`           | int                        | (None - uses `/cluster/nextid`)          | Start (inclusive) of a dedicated VMID range for this manager. Must be set together with `vmid_range_end`, must not contain `template_id`, and is recommended whenever more than one manager targets the same Proxmox VE cluster, since it stops just-freed IDs being reissued immediately and avoids collisions between managers. |
| `vmid_range_end`             | int                        | (None - uses `/cluster/nextid`)          | End (exclusive) of a dedicated VMID range for this manager. Must be set together with `vmid_range_start`.                                                                                                                                                                                                            |
