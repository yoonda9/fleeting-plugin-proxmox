...
| `proxmox_task_wait_interval` | int                       | 10                                      | How often to check for Proxmox task completion. Must be a positive number of seconds: the task wait now polls on a ticker, where a zero previously reached a `time.Sleep(0)` and busy-polled the API.                                                                                                                                                                                                                                                                       |
...
| `instance_connect_timeout`   | int (seconds)             | 60                                      | How long to wait for a newly deployed instance to report a usable network address. This bounds the whole wait as wall-clock time; the fixed 12-retry loop it replaces slept 60s in total *plus* one API round trip per retry, so raise this if instances on a loaded cluster are slow to get an address.                                                                                                                                                                                                                                  |
...
| `proxmox_api_retry_attempts` | int                        | 3                                        | How many times the heartbeat's guest-agent check is attempted in total, the first try included, while it keeps failing transiently - so `1` means no retry at all. No other read-only call is retried yet.                                                                                                                                                                                         |
