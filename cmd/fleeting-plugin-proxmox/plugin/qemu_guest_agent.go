...
func (ig *InstanceGroup) waitForAgent(ctx context.Context, vm *proxmox.VirtualMachine) error {
	...
	for {
		...
		// Waiting from the end of each poll, not tick to tick: AgentOsInfo blocks on
		// Proxmox's own QMP timeout while the agent is down, which routinely outlasts the
		// interval and would leave a ticker's tick already buffered - turning the wait into
		// back-to-back polls for its whole budget, on every instance of a batch at once.
		select {
		case <-ctx.Done():
			return agentStartTimeout(vm.VMID, lastErr, ctx.Err())
		case <-time.After(proxmox.DefaultWaitInterval):
		}
	}
}
