...
// vmidReleaseCheckTimeout bounds ReleaseIfFree's confirmation round trip. That check runs on
// a context detached from the caller's, so it needs a deadline of its own; half the default
// http_timeout leaves a loaded node room to answer while keeping a shutdown from stalling on
// a wedged pveproxy.
const vmidReleaseCheckTimeout = 30 * time.Second
...
func (a *vmidAllocator) ReleaseIfFree(ctx context.Context, vmid int) {
	...
	ctx, cancel := context.WithTimeout(context.WithoutCancel(ctx), vmidReleaseCheckTimeout)
	defer cancel()
