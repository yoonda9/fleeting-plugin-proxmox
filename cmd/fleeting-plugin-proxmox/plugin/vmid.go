...
// defaultVMIDReleaseCheckTimeout bounds ReleaseIfFree's confirmation round trip for an
// allocator built without one. That check runs on a context detached from the caller's, so it
// needs a deadline of its own; half of http_timeout leaves a loaded node room to answer while
// keeping a shutdown from stalling on a wedged pveproxy. clusterVMIDAllocator derives the live
// value from the setting, so raising http_timeout for a slow cluster also gives the
// confirmation room to answer rather than leaving it on a frozen copy of the default.
const (
	// vmidReleaseCheckTimeoutDivisor is the share of http_timeout the confirmation gets.
	vmidReleaseCheckTimeoutDivisor = 2

	defaultVMIDReleaseCheckTimeout = time.Duration(DefaultHTTPTimeout) * time.Second /
		vmidReleaseCheckTimeoutDivisor
)
...
// Proxmox API.
//
// mu guards reserved only, and is never held across a checkID or nextID call: a
// candidate is reserved optimistically under the lock, confirmed with the cluster
// unlocked, and given back if the cluster says it is taken. Holding it across the
// round trip would serialize every concurrent Allocate behind one HTTP call - on
// the path where the cluster is already slow.
type vmidAllocator struct {
	...
	// releaseCheckTimeout bounds ReleaseIfFree's confirmation round trip.
...
func newVMIDAllocator(
	...
	return &vmidAllocator{
		...
		checkID:             checkID,
		nextID:              nextID,
		releaseCheckTimeout: defaultVMIDReleaseCheckTimeout,
	}
}
...
func (a *vmidAllocator) Allocate(ctx context.Context) (int, error) {
	...
	for attempt := range vmidAllocateAttempts {
		...
		// The local reservation is taken first and given back if the cluster rejects
		// the candidate, so the checkID round trip runs unlocked while still
		// excluding every other caller from that same id.
		if !a.reserveLocally(vmid) {
			continue
		}
...
func (a *vmidAllocator) ReleaseIfFree(ctx context.Context, vmid int) {
	if !a.isReserved(vmid) {
		return
	}

	// The commonest reason a caller gets here at all is that ctx itself died - a runner
	// shutting down, or a provisioner deadline cancelling the clone wait. Confirming on
	// that same context would fail instantly with "context canceled", leaving the
	// reservation behind on exactly the path this exists to clean up, so the check runs
	// detached with a deadline of its own.
	ctx, cancel := context.WithTimeout(context.WithoutCancel(ctx), a.releaseCheckTimeout)
	defer cancel()

	// Safe to check unlocked: the reservation is what stops another Allocate from
	// handing out vmid while the round trip is in flight.
	free, err := a.checkID(ctx, vmid)
	if err != nil || !free {
		return
	}
...
// reserveLocally claims vmid in the reservation set, reporting false if another caller
// already holds it.
func (a *vmidAllocator) reserveLocally(vmid int) bool {
	a.mu.Lock()
	defer a.mu.Unlock()
...
// isReserved reports whether vmid is currently reserved by this allocator.
func (a *vmidAllocator) isReserved(vmid int) bool {
	a.mu.Lock()
	defer a.mu.Unlock()

	_, reserved := a.reserved[vmid]
