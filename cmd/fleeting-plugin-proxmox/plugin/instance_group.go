...
func (ig *InstanceGroup) clusterVMIDAllocator() *vmidAllocator {
	...
	allocator := newVMIDAllocator(cluster.CheckID, cluster.NextID)
	// Half of http_timeout, so the confirmation follows the setting instead of a frozen copy
	// of its default: the round trip is bounded by http_timeout anyway, and this leaves a
	// shutdown from stalling on a wedged pveproxy.
	allocator.releaseCheckTimeout = seconds(ig.HTTPTimeout) / vmidReleaseCheckTimeoutDivisor
