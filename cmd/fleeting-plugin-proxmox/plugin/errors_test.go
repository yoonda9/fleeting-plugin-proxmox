...
// transientHTMLHandler answers as an overloaded proxy does: a 503 with an HTML body that
// go-proxmox cannot decode, which classifyError must still call transient.
func transientHTMLHandler(w http.ResponseWriter, _ *http.Request) {
	w.WriteHeader(http.StatusServiceUnavailable)
	fmt.Fprint(w, "<html>Service Unavailable</html>")
}

// terminalBadRequestHandler answers with a 400 and Proxmox's error envelope, which
// classifyError calls terminal. A bare 500 would not do: that is transient.
func terminalBadRequestHandler(w http.ResponseWriter, _ *http.Request) {
	w.WriteHeader(http.StatusBadRequest)
	fmt.Fprint(w, `{"data":null,"errors":{"foo":"bar"}}`)
}
...
func TestClassifyError(t *testing.T) {
	...
	malformedBodyErr := realClassifyErrorFixture(t, transientHTMLHandler, 0)
	badRequestErr := realClassifyErrorFixture(t, terminalBadRequestHandler, 0)
