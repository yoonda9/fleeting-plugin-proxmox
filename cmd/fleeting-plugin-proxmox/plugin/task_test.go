...
func testUPID(taskType string) proxmox.UPID {
	return proxmox.UPID(fmt.Sprintf("UPID:%s:00001A2B:00000000:00000000:%s:100:root@pam:", increaseTestNode, taskType))
}
...
func taskStatusBody(taskType, status, exitStatus string) string {
	return fmt.Sprintf(`{"data":{"upid":%q,"node":%q,"type":%q,"id":"100","user":"root@pam","status":%q,"exitstatus":%q}}`,
		testUPID(taskType), increaseTestNode, taskType, status, exitStatus)
}
