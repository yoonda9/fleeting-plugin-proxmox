...
func classifyError(err error) errorClass {
	...
	if strings.Contains(msg, agentNotRunningMessage) {
	...
	if strings.HasPrefix(msg, "500 ") || strings.HasPrefix(msg, "501 ") {
		return classTransient
	}

	// netErr covers everything the HTTP round trip itself can fail with: connection
	// refused/reset, DNS failures, TLS handshake errors, and any timeout - including
	// http.Client.Timeout and a caller ctx deadline, both of which net/http surfaces as
	// a *url.Error implementing net.Error.
	//
	// syntaxErr and typeErr cover an overloaded pveproxy (or a proxy in front of it)
	// returning a non-JSON body - typically an HTML error page - for a status
	// go-proxmox otherwise treats as successful, which surfaces here as a raw json
	// decode error.
	var (
		netErr    net.Error
	...
	if errors.As(err, &netErr) || errors.As(err, &syntaxErr) || errors.As(err, &typeErr) {
		return classTransient
	}
